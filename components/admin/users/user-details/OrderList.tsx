            ...
            <div className="space-y-1">
              {order.items.map((item, i) => (
                <div key={i} className="flex justify-between text-xs text-gray-500">
                  ...
                  <span className="font-medium text-gray-700">{(item.price * item.quantity).toLocaleString("vi-VN")}đ</span>
                </div>
              ))}
            </div>
            {order.voucherCode && (
              <p className="text-xs text-green-600 font-medium mt-2"> Voucher: {order.voucherCode}</p>
            )}
          </div>
        );
      })}
