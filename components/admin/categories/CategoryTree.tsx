...
import { countDescendants } from "@/lib/categoryTree";
...
function depthColor<T extends readonly string[]>(arr: T, depth: number): T[number] {
...
function TreeNode({
  ...
  return (
    <div className={cn("border-l-2", node.depth > 0 ? `ml-3 ${borderColor}` : "border-l-transparent")}>
      {/* Row */}
      <div
        className={cn(
      ...
      >
        ...
        <button
          ...
          className={cn(
            ...
            hasChildren ? "text-gray-400 hover:text-gray-700" : "text-transparent cursor-default"
          )}
        >
          {hasChildren
            ? expanded
        ...
        {/* ID badge */}
        <span className={cn("flex-shrink-0 text-xs px-1.5 py-0.5 rounded font-mono", badgeColor)}>
          #{node.id}
        </span>
      ...
      {confirming && (
        <DeleteConfirmRow
          ...
          onConfirm={() => { setConfirming(false); onDelete(node); }}
